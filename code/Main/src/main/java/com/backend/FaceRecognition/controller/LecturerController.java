...
public class LecturerController {
    ...
    public ResponseEntity<Response> updateSubject(@RequestBody SubjectRequest request) {
        return build(lecturerService.updateSubject(request));
    }
    @PostMapping("/add-subject")
    public ResponseEntity<Response> addSubject(@RequestBody SubjectRequest request) {
        return build(lecturerService.addSubject(request));
    }
    ...
    public ResponseEntity<Response> updateAttendanceStatus(@PathVariable("subject_code")
                                                        String subjectCode,
    ...
    public ResponseEntity<Response> clearSubjectStudents(@RequestParam("subject_code") String subjectCode, @RequestHeader("Authorization") String bearer) {
    ...
    @PostMapping("/suspend")
    public ResponseEntity<Response> suspendStudentFromMarkingAttendance(@RequestParam("subject_code") String subjectCode,
    ...
    private ResponseEntity<StudentAttendanceRecordResponse>
        viewAttendanceRecord(@RequestHeader("Authorization") String bearer,
                         @RequestParam String studentId,
                         String subjectCode) {
        return lecturerService.viewAttendanceRecord(bearer,studentId,subjectCode);
    }
    @GetMapping("/print-record/{subject_code}")
    public ResponseEntity<ByteArrayResource> excelSheet(@PathVariable("subject_code") String subjectCode,
                                                        @RequestParam("date") String date,
                                                        @RequestParam("sort_id") int id,
                                                        @RequestHeader("Authorization") String bearer) {
        LocalDate localDate = LocalDate.parse(date);
